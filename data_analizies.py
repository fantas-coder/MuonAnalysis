"""
=============================================================
    Анализ базы данных ядерных эмульсий
...
def data_path(npl: str, binning: str, filename: str) -> Path:
    """
    Функция возвращает путь к файлам данных
    """
...
def load_detectors(npl: str = "npl4", binning: str = "1.0Grad") -> np.ndarray:
    """
    Detectors.txt — столбцы: [номер, X, Y, высота]
    (0,0) = детектор №9 (начало отсчёта)
    """
...
def load_input(npl: str = "npl4", binning: str = "1.0Grad") -> np.ndarray:
    """
    Input.txt — столбцы: [номер, X, Y, высота, угол_поворота°]
    Пропускает строки с заголовками.
    """
    rows = []
...
def load_efficiency(npl: str = "npl4", binning: str = "1.0Grad") -> np.ndarray:
    """
    EffCorFile_Tracks.dat — столбцы:
      [0] θ_D (градусы)
      [1] θ_D (радианы)
      [2] количество треков в бине
      [3] поправка на эффективность
    """
    return np.loadtxt(data_path(npl, binning, "EffCorFile_Tracks.dat"))


def load_tracks(npl: str, binning: str, det_num: int) -> np.ndarray | None:
    """
    Tracks_DistrOutput_N.dat — столбцы: [θ-бин, φ-бин, N_треков]
    Возвращает None если файл не существует или данные аномальны.
    """
    path = data_path(npl, binning, f"Tracks_DistrOutput_{det_num}.dat")
    if not path.exists():
        return None
    data = np.loadtxt(path)

    # Проверка аномального формата (npl6/1.0Grad — только 2 значения φ)
...
def sum_all_detectors(npl: str, binning: str, n_det: int = 18) -> np.ndarray | None:
    """
    Суммирует треки по всем детекторам в единую (θ,φ)-матрицу.
    """
    total = None
...
def make_grid(data: np.ndarray, theta_max: float = 90.0):
    """
    Из плоского массива (θ,φ,N) строит 2D-сетку для pcolormesh.
    Возвращает (phi_bins, theta_bins, grid).
    """
    sub = data[(data[:, 0] > 0) & (data[:, 0] <= theta_max)]
...
def theta_marginal(data: np.ndarray, theta_max: float = 90.0):
    """
    Интегральное распределение по θ (сумма по φ).
    """
    sub = data[(data[:, 0] > 0) & (data[:, 0] <= theta_max)]
    t_bins = np.unique(sub[:, 0])
...
def phi_marginal(data: np.ndarray, theta_max: float = 60.0):
    """
    Интегральное распределение по φ (сумма по θ до theta_max).
    """
    sub = data[(data[:, 0] > 0) & (data[:, 0] <= theta_max)]
    p_bins = np.unique(sub[:, 1])
...
def check_data_integrity() -> dict:
    """
    Проверяет наличие и формат всех файлов.
    ...
    for npl in ["npl3", "npl4", "npl5", "npl6"]:
        for binning in VALID_BINNINGS:
            ...
            if not folder.exists():
                ...
                continue
            tracks_files = list(folder.glob("Tracks_DistrOutput_*.dat"))
            if not tracks_files:
                status[(npl, binning)] = "NO_FILES"
                continue
            # Проверяем первый файл на аномалии
            d = np.loadtxt(tracks_files[0])
            n_phi = len(np.unique(d[:, 1]))
            if n_phi < 10:
                status[(npl, binning)] = f"ANOMAL (phi={n_phi})"
            else:
                status[(npl, binning)] = f"OK ({len(tracks_files)} дет.)"
    return status
...
def main():
    ...
    print("\n✓ Готово! Все графики сохранены в:", OUTPUT_DIR.resolve())
    plt.show()
