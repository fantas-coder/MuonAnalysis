...
# ─────────────────────────────────────────────────────────────────────────────
# ПАРАМЕТРЫ СГЛАЖИВАНИЯ (используются в muon_smoothing.py)
# ─────────────────────────────────────────────────────────────────────────────

# Гауссовский фильтр: стандартные отклонения по θ и φ в единицах бинов.
# Оптимальные параметры по результатам работы Мартьяновой О.А. (2026):
#   σ_θ=1.0, σ_φ=0.5 — лучший баланс шум/детали для биннинга 1.5°
# Для нашего биннинга 2.0° нужно проверить σ ∈ [0.5, 1.5].
GAUSS_SIGMA_THETA: float = 1.0
GAUSS_SIGMA_PHI:   float = 0.5

# Градиентный фильтр: размер локального окна усреднения.
# 3 = окно 3×3 (стандарт). Увеличение до 5 даёт более сильное сглаживание.
GRAD_WINDOW: int = 3

# Вейвлет BayesShrink: число уровней разложения.
# 1 — слабое (убирает только мелкий шум), 4 — сильное (рекомендуется).
WAVELET_LEVELS: int = 4
WAVELET_TYPE:   str = "db1"   # вейвлет Добеши первого порядка

# ─────────────────────────────────────────────────────────────────────────────
# ПАРАМЕТРЫ ГЕНЕРАТОРА (muon_synth.py) (используются в muon_synth.py)
# ─────────────────────────────────────────────────────────────────────────────

# Размер угловой сетки (соответствует биннингу 2.0°)
SYNTH_N_THETA: int = 17    # θ-бинов в диапазоне [58°, 90°]
SYNTH_N_PHI:   int = 180   # φ-бинов в диапазоне [0°, 360°]

# Диапазон суммарной статистики на один образец (в единицах нормированных треков)
# Соответствует диапазону значений после per_detector нормализации
SYNTH_INTENSITY_RANGE: tuple[float, float] = (0.3, 1.0)

# Диапазон числа аномалий (гауссовских пиков) на образец
SYNTH_N_ANOMALIES_RANGE: tuple[int, int] = (0, 3)

# Масштаб пуассоновского шума: λ = N/scale.
# Чем меньше scale -- тем сильнее шум. scale=0.005 -- уже очень сильный.
# Реалистичный диапазон для наших данных: 0.05–0.15.
SYNTH_NOISE_SCALE_RANGE: tuple[float, float] = (0.01, 0.04)

# Амплитуда азимутальных вариаций (геологическая неоднородность)
# 0 = идеально однородная азимутальная структура
...
# Аномалии размещаются только в нижней части θ-сетки (θ < 70°, индексы 0–5)
# — именно там они физически ожидаемы для почти горизонтальных мюонов
SYNTH_ANOMALY_THETA_IDX_RANGE: tuple[float, float] = (0.0, 5.0)

# ─────────────────────────────────────────────────────────────────────────────
# ПАРАМЕТРЫ ОБУЧЕНИЯ U-NET  (используются в muon_unet.py)
# ─────────────────────────────────────────────────────────────────────────────

# Архитектура
BASE_CHANNELS: int = 32    # число каналов на первом уровне; удваивается вглубь
DEPTH:         int = 3     # число уровней энкодера/декодера

# Обучение
PRETRAIN_EPOCHS:  int   = 50
FINETUNE_EPOCHS:  int   = 100
PRETRAIN_LR:      float = 1e-3
FINETUNE_LR:      float = 1e-5   # меньше LR при файнтюнинге
BATCH_SIZE:       int   = 32
WEIGHT_DECAY:     float = 1e-4

# Заморозка энкодера на первые N эпох файнтюнинга.
# Сохраняет представления, выученные при предобучении на синтетике.
FINETUNE_FREEZE_EPOCHS: int = 20

# Взвешенная функция потерь: вес ненулевых бинов
NONZERO_WEIGHT:   float = 5.0

# Разбивка реальных детекторов
TRAIN_DETS = [1, 2, 3, 5, 6, 7, 9, 10, 11, 12]
VAL_DETS   = [13, 14]
TEST_DETS  = [17, 18]
